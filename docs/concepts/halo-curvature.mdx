...
Same as the kerf open & close example, plus `cell_size`:
...
```json
{
  ...
  "xgr": [[10, 10, -27, 3]],
  "ygr": [],
  "core": { "E": 130e6, "nu": 0.3, "rho": 100, "cell_size": 0.6 },
  "resin": { "E": 3e9, "nu": 0.35, "rho": 1100 },
  "curvature": { "kx": 0.012, "ky": 0.0 },
  "scoring": { "damage_cells": 1.0, "surfaces": { "face": { "enabled": false } } },
  "backend": "numpy"
}
```

Top-mouth (`depth < 0`): **`kx > 0` opens**, `kx < 0` closes — same as
[Kerf open & close](/docs/concepts/curvature). Drop `cell_size` for sharp kerf
only; drop `curvature` for halo on a prismatic groove.

Reference: `examples/curved_panel/base.json` + `cell_size`. Composition tests:
`tests/test_halo_curvature.py`. Parametric stiffness:
