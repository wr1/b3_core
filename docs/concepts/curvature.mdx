...
## Curved on the mould → flattened for FEA

Same kerf taper, two presentations of `κ`:

| Panel | Geometry | Role |
|-------|----------|------|
| **1 · Curved** | Flattened FEA mesh **rolled onto a cylinder** (`θ = κ·x`, `r = R + z`) | Viz of the mould configuration — tip bluntness matches FEA |
| **2 · Flattened RVE** | Interval-affine morph: trapezoidal foam, walls track `hw(z)` | What `cprop` / `homogenize` actually solves |

Homogenization never meshes the macro bend. It uses the flat morphed RVE; the
downstream shell model puts curvature back via geometry. The curved panel is
that same mesh rolled onto an arc so open/closed tips look identical on both
sides (no extra pinching from rigid-block hinging).

**Open** (`kx > 0` — kerfs flare at the free face):

<img
  src="/figures/groove_curved_vs_flat.png"
  alt="Open curvature: left, FEA mesh rolled onto mould arc; right, flattened FEA RVE with trapezoidal foam and open kerf taper."
...
<img
  ...
  alt="Closed curvature: left, FEA mesh rolled onto mould arc with pinched kerfs; right, flattened FEA RVE with closed kerf taper."
  className="my-6 w-full rounded-lg border border-fd-border"
/>
...
render rolls that same mesh onto a cylinder for side-by-side viz.
...
Figures are **2D side views**. Curved panel is the FEA mesh rolled onto an arc;
flattened panel is the same morph (`examples/curved_panel/render.py`).
