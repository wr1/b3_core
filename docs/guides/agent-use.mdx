...
|------|------------|---------------|
| Resin-rich band from opened cells | `grid_scored(cell_size=…)` or `.with_halo(…)` | [Resin halo](/docs/concepts/resin-halo) (graphics) |
| Kerf open/close on a mould | `curved_panel(kx=…)` / `.with_curvature` | [Kerf open & close](/docs/concepts/curvature) |
| Both together / κ mass lookup | `curved_panel(...).with_halo(...)`; physics surrogate | [Halo + curvature](/docs/concepts/halo-curvature) |
