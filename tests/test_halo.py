...
def _inp(cell_size):
    return {"dx": 30, "dy": 30, "thickness": 20, "xgr": XG, "ygr": YG,
            "core": {**FOAM, "cell_size": cell_size}}


def _mesh(s_halo):
    return create_grooved_mesh(thickness=20, dx=30, dy=30, xcuts=XG, ycuts=YG,
...
def _Ez(cell_size=None, strategy="exact"):
    if cell_size is None:
        ...
        inp = _inp(cell_size)
        C = aniso.runnumpy(_M, RESIN, inp["core"], score_field=ScoreField(inp),
                           scoring={"sampling": {"strategy": strategy}}).stiffness
